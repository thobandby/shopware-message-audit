const v="modulepreload",w=function(e){return window.__sw__.assetPath+"/bundles/messengerhistorydashboard/administration/"+e},p={},b=function(t,a,h){let m=Promise.resolve();if(a&&a.length>0){let r=function(i){return Promise.all(i.map(o=>Promise.resolve(o).then(c=>({status:"fulfilled",value:c}),c=>({status:"rejected",reason:c}))))};document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),n=(s==null?void 0:s.nonce)||(s==null?void 0:s.getAttribute("nonce"));m=r(a.map(i=>{if(i=w(i),i in p)return;p[i]=!0;const o=i.endsWith(".css"),c=o?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${i}"]${c}`))return;const l=document.createElement("link");if(l.rel=o?"stylesheet":v,o||(l.as="script"),l.crossOrigin="",l.href=i,n&&l.setAttribute("nonce",n),document.head.appendChild(l),o)return new Promise((u,g)=>{l.addEventListener("load",u),l.addEventListener("error",()=>g(new Error(`Unable to preload CSS for ${i}`)))})}))}function d(r){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=r,window.dispatchEvent(s),!s.defaultPrevented)throw r}return m.then(r=>{for(const s of r||[])s.status==="rejected"&&d(s.reason);return t().catch(d)})},f=Shopware.Classes.ApiService;class y extends f{constructor(t,a){super(t,a,"mh-dashboard")}listMessages({status:t=null,searchTerm:a="",page:h=1,limit:m=25,topicGroup:d="",createdFrom:r="",createdTo:s=""}={}){const n=new URLSearchParams;t&&n.set("status",t),a&&n.set("query",a),n.set("page",h),n.set("limit",m),d&&n.set("topicGroup",d),r&&n.set("createdFrom",r),s&&n.set("createdTo",s);const i=n.toString();return this.httpClient.get(`/_action/mh/messages${i?`?${i}`:""}`,{headers:this.getHeaders()})}loadMetrics(){return this.httpClient.get("/_action/mh/metrics",{headers:this.getHeaders()})}getMessageDetail(t){return this.httpClient.get(`/_action/mh/messages/${encodeURIComponent(t)}`,{headers:this.getHeaders()})}getHeaders(){return{Accept:"application/json",Authorization:`Bearer ${this.loginService.getToken()}`,"Content-Type":"application/json"}}retryMessage(t,a="manual retry"){return this.httpClient.post(`/_action/mh/messages/${encodeURIComponent(t)}/retry`,{reason:a},{headers:this.getHeaders()})}quarantineMessage(t,a="manual quarantine"){return this.httpClient.post(`/_action/mh/messages/${encodeURIComponent(t)}/quarantine`,{reason:a},{headers:this.getHeaders()})}dismissMessage(t,a="manual dismiss"){return this.httpClient.post(`/_action/mh/messages/${encodeURIComponent(t)}/dismiss`,{reason:a},{headers:this.getHeaders()})}cleanupMessages(t=30){return this.httpClient.post("/_action/mh/cleanup",{days:t},{headers:this.getHeaders()})}}Shopware.Service().register("mhDashboardApiService",()=>new y(Shopware.Application.getContainer("init").httpClient,Shopware.Service("loginService")));const{Component:A}=Shopware;A.register("mh-dashboard-index",{inject:["mhDashboardApiService"],mixins:[Shopware.Mixin.getByName("notification")],template:`
        <sw-page class="mh-dashboard-index">
            ...
            <template #content>
                ...
                <sw-card title="Messages" style="margin-top:16px;">
                    ...
                    <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr auto;gap:16px;align-items:end;margin-bottom:16px;">
                        <sw-text-field
                        ...
                        <sw-button variant="primary" @click="applyFilters">Anwenden</sw-button>
                    </div>

                    <div style="display:grid;grid-template-columns:1fr auto auto;gap:16px;align-items:end;margin-bottom:16px;">
                        ...
                        <sw-single-select
                            ...
                            label="Alte Einträge löschen"
                            :options="cleanupOptions">
                        </sw-single-select>

                        <sw-button variant="danger" @click="cleanupOldEntries">Bereinigen</sw-button>
                    </div>
